...
class CustomDataset(Dataset):
    ...
    def __init__(self,confPath):
        ...
        self.loadingMemFeat(self.trainSubGTrack[self.subGptr//self.partNUM][self.subGptr%self.partNUM],0)
...
########################## 加载/释放 图结构数据 ##########################
    ...
    def initNextGraphData(self):
        ...
        halostart = time.time()
        haloend = time.time()
        self.loadingMemFeat(self.nextGID,self.graphNodeNUM)
    ...
    def initFeatCache(self):
        folder_path = self.dataPath  # 替换为你的文件夹路径
        target_file_name = 'range.bin'  # 替换为你的目标文件名
        nodeNUMList = []
        for root, dirs, files in os.walk(folder_path):
            if target_file_name in files:
                target_file_path = os.path.join(root, target_file_name)
                # 执行文件值数量的操作，这里使用前面提到的方法
                file_size = os.path.getsize(target_file_path)
                int32_size = 4
                nodeNUM = int((file_size // int32_size) / 2)
                nodeNUMList.append(nodeNUM)
        nodeNUMList.sort(reverse=True)
        featLen = nodeNUMList[0] + nodeNUMList[1]
        featLen = featLen * self.self.featlen
        self.feats = torch.zeros(featLen, dtype=torch.float32).reshape(-1,self.featlen)
...
########################## 特征提取 ##########################
    #@profile(precision=4, stream=open('./info.log','w+'))
    def loadingMemFeat(self,rank,ptr):
        filePath = self.dataPath + "/part" + str(rank)
        print("loading :",rank)
        self.feats[ptr:] = torch.from_numpy(np.fromfile(filePath+"/feat.bin", dtype=np.float32)).reshape(-1,self.featlen)
...
if __name__ == "__main__":
    dataset = CustomDataset("test.json")
