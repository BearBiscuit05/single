...
class GAT(nn.Module):
    def __init__(self, in_size, hid_size,heads=[1,1]):
        super().__init__()
        ...
        self.layers.append(dglnn.GATConv(hid_size*heads[0], hid_size, heads[1], feat_drop=0.6, attn_drop=0.6, activation=None,allow_zero_in_degree=True))
...
def train(args, device, g, reverse_eids, seed_edges, model):
    ...
    sampler = NeighborSampler([15, 10], prefetch_node_feats=["feat"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",default="mixed",choices=["cpu", "mixed", "puregpu"],
    ...
    # print("Validation MRR {:.4f}, Test MRR {:.4f}".format(
