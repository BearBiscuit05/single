...
########################## 加载/释放 图结构数据 ##########################
    ...
    def initNextGraphData(self):
        ...
        # 查看是否需要释放
        if self.subGptr > 0:
            print("before moving feats:",self.feats.shape)
            self.moveGraph()
            print("after moving feats:",self.feats.shape)
        ...
        haloend = time.time()
        print("initNextGraphData after prefetch next feats:",self.feats.shape)
        if prefetch:
    ...
    def preloadingGraphData(self):
        ...
        self.pregraph_src = srcdata
        self.pregraph_range = rangedata
        self.prefeat = np.fromfile(filePath+"/feat.bin", dtype=np.float32)


    def loadingGraph(self,merge=True,preFetch=False):
        """
        merge 表示此处需要融合两个图结构
        ...
        if preFetch == False:
            ...
            self.pregraph_src = torch.tensor(self.pregraph_src,device=('cuda:%d'%self.cudaDevice))
            self.pregraph_range = torch.tensor(self.pregraph_range,device=('cuda:%d'%self.cudaDevice))
            self.cacheData[0] = torch.cat([self.cacheData[0],self.pregraph_src])
            self.cacheData[1] = torch.cat([self.cacheData[1],self.pregraph_range])
...
########################## 采样图结构 ##########################
    ...
    def getNegNode(self,sampleIDs,batchlen,negNUM=1):
        ...
        neg_dst = torch.randint(low=0, high=self.graphNodeNUM, size=raw_src.shape).to(torch.int32).to("cpu")
...
########################## 特征提取 ##########################
    ...
    def loadingMemFeat(self,rank,preFetch=False):
        
        # TODO:如果preFetch为True,则直接从指定位置加载
        if preFetch:
            print("loadingMemFeat func with preFetch == TRUE -> self.feats :",self.feats.shape)
        filePath = self.dataPath + "/part" + str(rank)
        if preFetch == False:
        ...
        if self.feats == []:
            ...
            if preFetch == False:
                ...
                print("self.prefeat :",self.prefeat.shape)
                print("self.feats :",self.feats.shape)
                #self.prefeat = torch.from_numpy(self.prefeat).reshape(-1,self.featlen).to("cpu")
                tmp_feat = copy.deepcopy(self.prefeat)
                tmp_feat = torch.from_numpy(tmp_feat).reshape(-1,self.featlen)#.to("cpu")
                print(tmp_feat.shape)
                self.feats = torch.cat([self.feats,tmp_feat.to("cpu")])
        if preFetch:
            print("after loadingMemFeat func -> self.feats :",self.feats.shape)
