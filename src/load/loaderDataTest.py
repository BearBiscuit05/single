...
########################## 加载/释放 图结构数据 ##########################
    def initNextGraphData(self):
        ...
        self.loadingHalo()
        haloTime = time.time() - haloStartTime
        
        featStartTime = time.time()
        self.loadingMemFeat(self.nextGID)
        featTime = time.time() - featStartTime

        # Test 
        print("训练图:{},下一图:{},图训练集规模:{},图节点数目:{},图边数目:{}, \
                整体耗时:{}s,图移动:{}s,图加载:{}s,halo:{}s,特征:{}s"\
                        .format(self.trainingGID,self.nextGID,self.subGtrainNodesNUM,self.graphNodeNUM,\
                        self.graphEdgeNUM,time.time()-start,moveTime,loadingTime,haloTime,featTime))
    ...
    def moveGraph(self):
        ...
        # Test
        if self.cacheData[1][0] != 0 or self.cacheData[1][-1] > len(self.cacheData[0]):
            print("move graph have error...")
            print("range start:{}, max range{}, right range:{}".format(self.cacheData[1][0],self.cacheData[1][-1],len(self.cacheData[0])))
            exit(-1)
...
########################## 采样图结构 ##########################
    ...
    def preGraphBatch(self):
        ...
        print("sample graph time :{}".format(time.time() - sampleStartTime))
        ...
        # Test
        featMergeStartTime = time.time()
        cacheFeat = self.featMerge(cacheGraph,cacheFeat)
        print("graph feat merge time :{}".format(time.time() - featMergeStartTime))

        # Test
        blockStartTime = time.time()
        cacheGraph = self.transGraph2Block(cacheGraph)
        print("graph block create time :{}".format(time.time() - blockStartTime))
